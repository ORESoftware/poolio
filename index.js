...
const acceptableConstructorOptions = ['execArgv', 'args', 'size', 'filePath', 'addWorkerOnExit', 'cpOptions'];
...
function Pool(options) {
	...
	if (!path.isAbsolute(this.filePath)) {
		this.filePath = path.resolve(root + '/' + String(this.filePath));
	}
...
Pool.prototype.addWorker = function () {
	...
	const p1 = '/Users/Olegzandr/WebstormProjects/suman/test/suman/logs/test-stdout.log';
	...
	n.stdio[1].pipe(fs.createWriteStream(p1));
	n.stdio[2].pipe(fs.createWriteStream(p1));
